...
- 执行上游 retrieve_note 控制函数的两项 stub 检查；[输出](results/source-probe/probes.json)、[manifest](results/source-probe/manifest.json)。
...
这两项检查证明的是控制分支行为，不证明 note 的质量、检索提升或长期价值。受控表示实验尚未执行；后续使用独立 protocol 和结果目录，不倒填 v0.1.1 或原冻结 E1–E4。
